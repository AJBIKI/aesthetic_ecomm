...
import { dtoToProduct, volumeToCollection } from '@/lib/mappers';
...
export default async function CollectionDetailPage({ params }: PageProps) {
  ...
  // Try API for collections/volumes, fall back to static JSON
  const vols = await api.getVolumes();
  const collections: Collection[] = vols && vols.length > 0
    ? vols.map(volumeToCollection)
    : (collectionsData as Collection[]);

  // Try API for products, fall back to static JSON
  const dtos = await api.getProducts();
  const allProducts: Product[] = dtos && dtos.length > 0
  ...
  const collectionProducts = allProducts.filter(
    (p) => p.category?.toLowerCase().includes(collection.name.toLowerCase()) || p.collection === collection.slug
  );
  ...
  return (
    <div className="py-16 md:py-24 space-y-12 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center md:text-left space-y-4">
        ...
        <h1 className="text-4xl sm:text-5xl font-display text-[oklch(0.13_0.02_260)] tracking-wide"
